...
The `okf` command and regular `@okfxjs/core` APIs use TypeScript and work without
...
Backend packages are optional and must be installed alongside `@okfxjs/core`.
...
| Package | Target |
| --- | --- |
| `@okfxjs/core-darwin-arm64` | macOS arm64 |
| `@okfxjs/core-darwin-x64` | macOS x64 |
| `@okfxjs/core-linux-x64-gnu` | Linux x64 glibc |
| `@okfxjs/core-linux-x64-musl` | Linux x64 musl |
| `@okfxjs/core-win32-x64-msvc` | Windows x64 MSVC |
| `@okfxjs/native` | Portable/manual native package |
...
The primary WASM package is `@okfxjs/wasm`. `@okfxjs/core-wasm` is reserved as a
...
7. Publish platform packages before publishing `@okfxjs/core` and `okfx`.

Native and WASM packages must use the same version as `@okfxjs/core`. If no optional
